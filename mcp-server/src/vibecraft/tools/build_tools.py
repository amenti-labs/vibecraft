...
async def handle_build(
    ...
    if code:
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"❌ Unexpected error executing code: {type(e).__name__}: {e}"
            )]
        ...
        if not isinstance(commands, list):
            return [TextContent(
                type="text",
                text="❌ Error: commands must be a list of strings"
            )]
    ...
    for i, cmd in enumerate(commands):
        if not isinstance(cmd, str):
            return [TextContent(
                type="text",
                text=f"❌ Error: command at index {i} is not a string"
            )]
    ...
    if preview_only:
        ...
        if command_count <= 20:
            ...
            result_lines.append("```")
            result_lines.append("")
            result_lines.append(f"💡 Set preview_only=false to execute all {command_count} commands")
    ...
    for i, cmd in enumerate(commands):
        try:
            ...
            if result and any(err_word in result.lower() for err_word in ["error", "unknown", "incorrect", "invalid", "cannot"]):
            ...
            if (i + 1) % 50 == 0 or (i + 1) == command_count:
                ...
                result_lines.append(
                    f"  [{commands_executed}/{command_count}] {progress_pct:.1f}%"
                )
